...
pub fn load_system_config() -> Result<SystemConfig, ConfigError> {
    ...
    let config_path = "/etc/os_gaming/config.toml";
    
    // Create a new filesystem manager
    let fs_manager = filesystem::FilesystemManager::new();
    
    match fs_manager.open_file(config_path, true) { // true for readonly
        Ok(mut file) => {
            // Create a buffer to hold file content
            let mut buffer = Vec::new();
            buffer.resize(4096, 0); // Initial buffer size of 4KB
            ...
            loop {
                match file.read(&mut buffer[total_read..], &fs_manager, 4096) {
                    Ok(bytes_read) => {
                        ...
                        // Ensure we have enough buffer space for the next read
                        if total_read == buffer.len() {
                            buffer.resize(buffer.len() * 2, 0);
                        }
                    },
                    Err(_) => return Err(ConfigError::IoError("Failed to read config file")),
                }
            }
            ...
            match String::from_utf8(buffer) {
                Ok(s) => content = s,
                Err(_) => return Err(ConfigError::IoError("Config file contains invalid UTF-8")),
            }
            
            // Parse TOML
            match toml::from_str(&content) {
                Ok(config) => Ok(config),
                Err(_) => Err(ConfigError::ParseError("Invalid config file format")),
            }
        },
        Err(_) => {
            ...
            // Try to save it
            if let Err(_) = save_system_config(&config) {
...
pub fn save_system_config(config: &SystemConfig) -> Result<(), ConfigError> {
    ...
    let toml = match toml::to_string(config) {
        Ok(toml) => toml,
        Err(_) => return Err(ConfigError::ParseError("Failed to serialize config")),
    };
    ...
    if let Err(_) = fs_manager.create_directory(dir_path) {
        log::warn!("Failed to create config directory");
    }
    
    // Write to file
    let config_path = "/etc/os_gaming/config.toml";
    match fs_manager.open_file(config_path, false) { // false for write mode
        Ok(mut file) => {
            ...
            while position < bytes.len() {
                match file.write(&bytes[position..], &fs_manager) {
                    ...
                    Err(_) => return Err(ConfigError::IoError("Failed to write config file")),
                }
            }
            
            Ok(())
        },
        Err(_) => Err(ConfigError::IoError("Failed to open config file for writing")),
    }
}

/// Global configuration
lazy_static! {
    static ref CONFIG: Mutex<SystemConfig> = Mutex::new(
        load_system_config().unwrap_or_else(|e| {
