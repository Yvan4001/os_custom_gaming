...
use crate::kernel::{self, interrupts, memory, drivers};
use crate::kernel::{drivers::filesystem as fs};
use crate::gui::{self, WindowManager, Renderer, Theme, FontManager};
use crate::config::{SystemConfig, load_system_config};
...
pub enum InputEvent {
    /// Keyboard event
    Key { key: u16, pressed: bool, modifiers: u8 },
    
    /// Mouse event
    Mouse { x: i32, y: i32, buttons: u8, scroll_delta: i8 },
    
    /// Gamepad event
    Gamepad { id: u8, buttons: u32, axes: [i16; 6] },
    
    /// Touch event
    Touch { id: u8, x: i32, y: i32, pressure: u8 },
}
...
impl System {
        ...
        if let SystemEvent::Input(input) = &event {
            if let Some(wm) = &self.window_manager {
                ...
                match input {
                    ...
                    InputEvent::Mouse { x, y, buttons, scroll_delta } => {
    ...
    fn save_system_state(&self) {
        ...
        if let Some(wm) = &self.window_manager {
            let window_layout = wm.lock().save_layout();
            
            // Save to config
            let mut config = self.config.lock();
            config.window_layout = Some(window_layout);
    ...
    fn show_error_screen(&self) {
        if let Some(wm) = &self.window_manager {
            ...
            // Create error dialog
            let window_id = wm.create_window("System Error", 400, 300, true).unwrap_or(0);
            if window_id > 0 {
